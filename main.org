...
- 第3节: [[id:easy-latex-writing-ep03-tab-completion][Tab 补全快速插入LaTeX代码]] ([[https://b23.tv/ypBKN9v][视频]])
...
:END:
*** ~TAB~ 补全插入宏命令模板
*原理*: 字母组合+ ~TAB~ 键生成 LaTeX 宏命令模板

**** 预设命令举例
保存在 =cdlatex-command-alist= 变量中

- 空格 ~qq~ + ~TAB~ = ~\quad~
- 分数 ~fr~ + ~TAB~ = ~\frac{}{}~
- 根号 ~sq~ + ~TAB~ = ~\sqrt{}~ 
- 括号 ~lr(~ + ~TAB~ = ~\left( \right)~ 
- 标题 ~sn~ + ~TAB~ = ~\section{}~ 
  
如果宏命令有(多个)参数在括号 ~{}~, ~[]~ 内, 补全时光标会自动停在第一个括号内, 第一个参数输入完毕后按 ~TAB~ 可以将光标跳到第二个括号内, 以次类推.


**** 一些自定义例子
- ~te~ + ~TAB~ = ~\text{}~
- ~se~ + ~TAB~ = ~\{ \}~ (set)
- ~hl~ + ~TAB~ = ~\hline~, ~hhl~ + ~TAB~ = ~\\ \hline~
- ~big(~ + ~TAB~ = ~\big( \big)~, ~lr<~ + ~TAB~ = ~\langle\rangle~
- ~st~ + ~TAB~ = ~\stackover{}{}~
...
#+begin_src latex
  ...
  \end{equation}
#+end_src
- ~NNN~ 是 =reftex= 自动生成的一串数字
- ~?~ 是光标停留的位置.

~enu~ + ~TAB~ 插入 (等同于 ~cdlatex-environment~ / ~C-c {~  + ~enumerate~)
#+begin_src latex
  \begin{enumerate}
  \item 
  \end{enumerate}
...
加入新的补全命令通过修改变量 =cdlatex-command-alist=.
**** 带参数宏命令
例子: ~te~ + ~TAB~ 输入 ~\text{?}~ (光标停在括号内)
- keyword: =te=
- Docstring: 随便填, 只是用于说明
- Replacement: =\text{?}= (? 表示光标停留的位置)
- Hook: =cdlatex-position-cursor= (需要指定光标必填!)
- Argument: =nil= (这是上面 =hook 的参数)
- Text Mode: =nil=, Math mode: =t=

保存设置后, 在已经打开的 =tex= 文件中用 ~C-c C-n~ 可以刷新设置.
**** 插入匹配的括号
例子: ~big{~ + ~TAB~ 插入 ~\big\{? \big\}~

这里有一个细节: =cdlatex= 中输入括号会自动匹配, 具体可以查看 =cdlatex-paired-parens=, 主要针对 ~()~, ~[]~, ~{}~

- keyword: =big{=
- Docstring: 随便填
- Replacement: =\big\{? \big\= (因为 ={}= 已经配对输入了, 所以这里要少输一个 =}=!)
...
~cdlatex-environment~ 会调用 =cdlatex-env-alist-comb= 中的环境模板, 因此 ~des~ + ~TAB~ 相当于 ~C-c {~ + ~description~

自定义环境模板可以修改 =cdlatex-env-alist=.

使用模板的好处:
- 支持自动插入标签: =AUTOLABEL= 关键字
- 支持多行环境的 =item= 模板 (~C-ENTER~ 触发)
*** 总结
- ~XXXX~ + ~TAB~ 可以快速输入复杂命令和环境, 自定义也很方便 (=cdlatex-command-alist=)
- 插入环境有两种方法:
  - 把环境模板放到 =cdlatex-command-alist= 中: 自由度大
  - 利用 =cdlatex-environment=, 把环境模板放到 =cdlatex-env-alist= 中: 支持标签、多行环境
- 更多例子可以参考我的 Emacs 配置: =gitee.com:mickey991/emacs-config.git=
下期: 如何高效交叉引用, 包括特殊的补全命令 ~lbl~ + ~TAB~, ~ref~ + ~TAB~ 以及 =refTeX= 配置.
